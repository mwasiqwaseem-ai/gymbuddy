...
import { Dumbbell, Phone, ArrowLeft, Zap } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      toast.success('Welcome to GymBuddy! Your 7-day trial has started 💪');
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      toast.error('Failed to send OTP. Check the number format.');
    } finally { setLoading(false); }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      toast.success('Account created! Your 7-day trial starts now 🔥');
      router.push('/dashboard');
    } catch {
  ...
  return (
    <div className="min-h-screen pt-16 flex items-center justify-center bg-gym-black relative overflow-hidden">
      ...
      <div className="absolute top-1/4 right-1/3 w-96 h-96 rounded-full bg-orange-500/3 blur-3xl" />
      ...
      <div className="relative w-full max-w-md px-4 py-12">
        ...
        <div className="text-center mb-10">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neon-green/10 text-neon-green text-sm font-medium mb-4">
            <Zap className="w-3.5 h-3.5" /> 7 Days Free Trial
          </div>
          <h1 className="text-3xl font-black mb-2">Join GymBuddy</h1>
          <p className="text-gray-500 text-sm">Create your account and start training for free</p>
        </div>

        {/* Card */}
        <div className="neon-card rounded-2xl p-6 border border-neon-green/10">
          {!showPhone ? (
            <>
              <button
            ...
            <div>
              ...
              {!confirmResult ? (
                <>
                  ...
                  />
                  <p className="text-xs text-gray-600 mb-4">Include country code. Pakistan: +92</p>
                ...
                <>
                  <p className="text-sm text-gray-400 mb-4">We sent a 6-digit code to {phone}</p>
                  ...
                  <input
                    ...
                    placeholder="• • • • • •"
                    maxLength={6}
                    className="w-full bg-gym-dark border border-gym-border rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-neon-green/50 mb-4 tracking-widest text-center text-xl"
                  />
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          ...
          <Link href="/login" className="text-neon-green hover:underline font-medium">Login</Link>
        </p>
      </div>
    </div>
  );
